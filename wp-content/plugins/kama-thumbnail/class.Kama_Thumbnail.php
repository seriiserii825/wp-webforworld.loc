...
class Kama_Thumbnail {
	...
	function __construct(){
		...
		self::$opt = (object) array_merge( self::def_options(), (array) $opts );

		// дополним опции. идет ниже определения опций
		...
		if( $ah && ! is_array($ah) ){
			...
			foreach( $ah as & $host ) $host = str_replace('www.', '', $host );
		}
		else
			$ah = array();
	...
	function wp_init(){
		...
		if( is_admin() && ! defined('DOING_AJAX') ){
			...
			add_action( $hook_name, array( $this, 'admin_options' ) ); // закомментируйте, чтобы убрать опции из админки
			add_action( $hook_name, array( $this, 'clear_handler' ) );
	...
	static function activation(){
		if( is_multisite() ){
			if( ! get_site_option(self::$opt_name) )
				update_site_option( self::$opt_name, self::def_options() );
		}
		else {
	...
	function admin_options(){
		...
		if( is_multisite() )
			$hook = add_submenu_page( 'settings.php', 'Kama Thumbnail', 'Kama Thumbnail', 'manage_network_options', self::$opt_pagename, array( &$this, 'network_options_page') );
		...
		register_setting( self::$opt_pagename, self::$opt_name, array( __CLASS__, 'sanitize_options') );
	}
	...
	function options_field(){
		...
		$opts = is_multisite() ? get_site_option( self::$opt_name ) : get_option( self::$opt_name );
		...
		$elems = array(
			'cache_folder' =>
				'<input type="text" name="'. $opt_name .'[cache_folder]" value="'. $opt->cache_folder .'" style="width:80%;" placeholder="'. $this->opt->cache_folder .'">'.
				'<p class="description">'. __('Full path to the cache folder with 755 rights or above.','kama-thumbnail') .'</p>',

			'cache_folder_url' =>
				'<input type="text" name="'. $opt_name .'[cache_folder_url]" value="'. $opt->cache_folder_url .'" style="width:80%;" placeholder="'. $this->opt->cache_folder_url .'">
				<p class="description">'. __('URL of cache folder.','kama-thumbnail') .'</p>',

			'no_photo_url' =>
				'<input type="text" name="'. $opt_name .'[no_photo_url]" value="'. $opt->no_photo_url .'" style="width:80%;" placeholder="'. $this->opt->no_photo_url .'">
			...
			'auto_clear' => '
				<label>
					...
					'. __('Clear all cache automaticaly every 7 days.','kama-thumbnail') .'
				</label>',
	...
	## sanitize options
	static function sanitize_options( $opts ){
		$defopt = (object) self::def_options();
	...
	function smart_clear( $type = '' ){
		...
		if( $expire < time() )
			$this->_clear_cache( $_stub ? 'only_stub' : '' );

		@ file_put_contents( $expire_file, time() + ($_stub ? DAY_IN_SECONDS : WEEK_IN_SECONDS) );
	}
	...
	function _clear_cache( $only_stub = '' ){
		...
		if( $only_stub ){
			foreach( glob("$cache_dir/stub_*") as $file ) unlink($file);
